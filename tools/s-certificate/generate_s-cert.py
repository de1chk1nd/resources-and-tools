...
L                      = Lindholm
O                      = de1chk1nd labs
OU                     = MCN Demo Facility
CN                     = %(domain)s
emailAddress           = de1chk1nd@outlook.com
...
def gencert(domain, rootdir=MYDIR, keysize=KEY_SIZE, days=DAYS,
    ...
    openssl ('pkcs12', '-export', '-inkey', dfile('key'), '-in', dfile('cert'), '-out', dfile('p12'))
    
    os.remove (str(dfile('key')))
    os.remove (str(dfile('request')))
    os.remove (str(dfile('cert')))
    os.remove (str(dfile('config')))

    print("Done - p12 cert file is located here: "+str(dfile('p12')))
